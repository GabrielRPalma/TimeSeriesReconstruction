...
proposed_approach_reconstructing_time_series_simulation <- function(n, TT, 
  ...
  for(i in weeks) {
    ...
    features <- c(features, colnames(train_dataset)[ -ncol(train_dataset)])
    feature_week <- c(feature_week, rep(i, length(colnames(train_dataset)[ -ncol(train_dataset)])))
    error <- c(error, rep(test_response_variable[index] - rf_model_reconstructed_predictions[index], length(colnames(train_dataset)[ -ncol(train_dataset)])))
    index <- index + 1
  }
  performance <- plot_and_check_performance(test_response_variable = 
                                            ...
                                            do_plot = FALSE)
  features <- features[-1]
  feature_week <- feature_week[-1]
  error <- error[-1]
  lags_used <- apply(as.matrix(features), MARGIN = 1, FUN = function(x){
    as.numeric(strsplit(x, split = "_lag.")[[1]][2])})
  feature_name <- apply(as.matrix(features), MARGIN = 1, FUN = function(x){
   strsplit(x, split = "_lag.")[[1]][1]})
  features_dataset <- data.frame(features_complete = features, 
  ...
  strsplit(features[2], split = "_lag.")
  result <- list()
  result$corr <- performance$obtained_cor
  result$rsme <- performance$obtained_smse
  result$predictions <- rf_model_reconstructed_predictions
  result$features <- features_dataset
...
get_learning_algorithms_performance <- function(train_dataset,
  ...
  if (method == 'Lasso'){
    if(dim(as.matrix(train_dataset[,-ncol(train_dataset)]))[2] ==1 ){
      ...
      if(sum(train_dataset$Target < 0) > 0){
...
obtain_all_approaches_performance <- function(data_source, 
  ...
  performance_dataset <- data.frame(Approach = NA, 
                                    ...
                                    Testing_samples = NA)
  reconstruction_features_dataset <- data.frame(Features = NA, 
                                                Method = NA, 
                                                Data_source = NA,
                                                Association = NA,
                                                Corr = NA, RSME = NA, 
  ...
  # Training with few samples (40 observations)
  ## Light GBM
  n <- 30
  ...
  reconstruction_features30 <- proposed_approach_reconstructing_time_series$features
  reconstruction_features30$Data_source <- rep(data_source, nrow(reconstruction_features30))
  reconstruction_features30$Association <- rep(association, nrow(reconstruction_features30))
  reconstruction_features30$Initial_training_samples <- rep(as.character(n), nrow(reconstruction_features30))
  ...
  obtained_performance <- data.frame(Approach = 'Time series reconstruction',
                                     ...
                                     Initial_training_samples = as.character(n), 
                                     Testing_samples = length(test_response_variable))
  reconstruction_features60 <- proposed_approach_reconstructing_time_series$features
  reconstruction_features60$Data_source <- rep(data_source, nrow(reconstruction_features60))
  reconstruction_features60$Association <- rep(association, nrow(reconstruction_features60))
  reconstruction_features60$Initial_training_samples <- rep(as.character(n), nrow(reconstruction_features60))
  ...
  results <- list()
  results$performance_dataset <- performance_dataset
  results$reconstructed_features <- rbind(reconstruction_features30, 
                                          reconstruction_features60)
